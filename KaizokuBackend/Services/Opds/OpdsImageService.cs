...
public class OpdsImageService
{
        ...
        CancellationToken token = default)
    {
        ...
        // series.StoragePath is relative (e.g. "Manhwa/A Bad Person") —
        // resolve to absolute once and use everywhere
        string storageFolder = _configuration.GetValue<string>("StorageFolder", "");
        string seriesStoragePath = string.IsNullOrWhiteSpace(storageFolder)
            ? series.StoragePath
            : System.IO.Path.Combine(storageFolder, series.StoragePath);

        string? archivePath = ResolveArchivePath(seriesStoragePath, chapterFilename);
        ...
        List<string> supportedImageFormats)
    {
        try
        {
            ...
            // Collect deduplicated chapters ordered ascending
    ...
    /// <summary>
    /// Resolves an archive file path from a series directory and chapter filename.
    /// The series directory path is expected to be absolute (caller resolves storageFolder).
    /// Tries exact match first, then by common extensions, then scans the directory.
    /// </summary>
    private static string? ResolveArchivePath(string seriesAbsolutePath, string chapterFilename)
    {
        if (!Directory.Exists(seriesAbsolutePath))
            return null;
