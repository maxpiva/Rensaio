...
import { usePendingRequestCount } from "@/lib/api/hooks/useRequests";
...
const ALL_NAV_ITEMS = [
  ...
  { name: "Requests", href: "/requests", icon: BookOpen },
...
// Hook to build permission-gated nav groups
function useNavGroups(): NavGroup[] {
  ...
  if (canViewLibrary) libraryItems.push({ name: "Library", href: "/library", icon: Library });
  if (canBrowse) libraryItems.push({ name: "Browse", href: "/cloud-latest", icon: Sparkles });

  const mgmtItems: NavItem[] = [];
  if (isAdmin) mgmtItems.push({ name: "Sources", href: "/providers", icon: Plug });
  if (canViewQueue) mgmtItems.push({ name: "Queue", href: "/queue", icon: List });
  mgmtItems.push({
    name: "Requests",
    href: "/requests",
    icon: BookOpen,
    badge: pendingCount && pendingCount.count > 0 ? pendingCount.count : undefined,
  });
  mgmtItems.push({ name: "Settings", href: "/settings", icon: LucideSettings });

  const groups: NavGroup[] = [];
  if (libraryItems.length) groups.push({ label: "Library", items: libraryItems });
  if (mgmtItems.length) groups.push({ label: "Management", items: mgmtItems });
  return groups;
}
...
function SidebarDownloadCounters({ expanded }: { expanded: boolean }) {
  ...
  if (!canViewQueue || !metrics) return null;
                  ...
                  return (
                    <Tooltip key={href} delayDuration={0}>
                      ...
                      <TooltipContent side="right" className="z-50">
                        {name}{badge && badge > 0 ? ` (${badge} pending)` : ''}
                      </TooltipContent>
                    </Tooltip>
                  );
                })}
