...
namespace RensaioBackend.Services.Helpers
{
    public class ArchiveHelperService
    {
        ...
        public async Task UpdateTitleAndAddComicInfoAsync(Guid seriesId, bool onlyDownloadByRensaio = true,
         CancellationToken token = default)
        {
            ...
            if (series == null)
                return;
            await UpdateTitleAndAddComicInfoAsync(series, onlyDownloadByRensaio, token).ConfigureAwait(false);
            await _db.SaveChangesAsync(token).ConfigureAwait(false);
        }
        public async Task UpdateTitleAndAddComicInfoAsync(Models.Database.SeriesEntity series, bool onlyDownloadByRensaio = true, CancellationToken token = default)
        {
            ...
            foreach (var sp2 in series.Sources)
            {
                ...
                foreach (var chap in sp.Chapters)
                {
                    ...
                    if (comicInfoName != null)
                    {
                        ...
                        {
                            if (oldComicInfo.Notes != null && oldComicInfo.Notes.Contains("Rensaio"))
                            {
            ...
            // Sync rensaio.json after archive rename or ComicInfo.xml update
